...
class Evaluator {
 public:
  Evaluator( const string &s ) : str( s ) { // constructor receives an exp
	...
	PREC_TABLE[26].inputSymbol = 1;    // ASSIGNMENT FOR PROGRAM 6, gave "=" a low priority since an assignment should come
	PREC_TABLE[26].topOfStack = 2;	   // at the end of everything that happens to the right of the '='.
  }
 ...
 private:
  ...
  NumericType getTopVar();					// Get top of var stack.
